MainFunction_JM = function(x,y,z,a,b,d,e) {
  ...
  # -----------------------------------------------------------------------------#
  if (e == 1){
    Dataset1 = read_excel(x, 
                          sheet = "CondA_Baseline")
    attach(Dataset1)
    
    
    Dataset3 = read_excel(x, 
                          sheet = "CondA_Stim")
    attach(Dataset3)
  }
  ...
  Baseline_Dataset$Time = (1:length(Baseline_Dataset$Beat_Num)) * .001
  Stressor_Dataset$Time = (1:length(Stressor_Dataset$Beat_Num)) * .001
  ...
  Baseline_Dataset$SplineMCAv = na.approx(Baseline_Dataset$LMCAv, x = Baseline_Dataset$Time)
  Baseline_Dataset$SplineMAP = na.approx(Baseline_Dataset$reBAP, x = Baseline_Dataset$Time)
  
 
  
  Stressor_Dataset$SplineMCAv = na.approx(Stressor_Dataset$LMCAv, x = Stressor_Dataset$Time)
  ...
  #---- Filter MCAv Through Median Filter ---- 
  ...
  #---- Filter MCAv and MAP Through Butterworth Filter ----
  ...
  # -----------------------------------------------------------------------------#
  Base_matrix = matrix(, ncol = 10, nrow= max(as.numeric(Baseline_Dataset$Beat_Num)))
  colnames(Base_matrix) = c("Beat", "CrCP", "RAP", "MCAv", "MAP", "Check", "Time", "CPI", "CPP", "CVCi")
  Base_matrix[1,7] = 0
  
  Stress_matrix = matrix(, ncol = 10, nrow= max(as.numeric(Stressor_Dataset$Beat_Num)))
  colnames(Stress_matrix) = c("Beat", "CrCP", "RAP", "MCAv", "MAP", "Check", "Time", "CPI", "CPP", "CVCi")
  Stress_matrix[1,7] = 0
  ...
  for(i in 1:max(Baseline_Dataset$Beat_Num)){ 
    ...
    CardiacCycleLength = length(PulledData$Sample) * 0.001
  ...
  Base_Matrix_Df[Base_Matrix_Df$CrCP < 0, c("CrCP", "RAP", "MAP", "MCAv", "CPP", "CPI", "CVCi")] = NA
  ...
  #StressT = seq(min(SubComp_Stress_matrix[,7]), max(SubComp_Stress_matrix[,7]), by = .2)
  ...
  for(i in 1:max(Stressor_Dataset$Beat_Num)){ 
    ...
    CardiacCycleLength = length(PulledData3$Sample) * 0.001
  ...
  Stress_Matrix_Df[Stress_Matrix_Df$CrCP < 0, c("CrCP", "RAP", "MAP", "MCAv", "CPP", "CPI", "CVCi")] = NA
...
#-------------------------------------------------------------------------------
# Create dataframe for TFA analysis. 
# ------------------------------------------------------------------------------
